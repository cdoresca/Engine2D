...
namespace _2D_engine.Figure
{
    internal class Triangle : Forme
    {
        ...
        public Triangle(double width, double height)
        {
            ...
            Algebre.Point min = new Point(Math.Min(p1[0], Math.Min(p2[0], p3[0])), Math.Min(p1[1], Math.Min(p2[1], p3[1])),
                Math.Min(p1[2], Math.Min(p2[2], p3[2])));

            Algebre.Point max = new Point(Math.Max(p1[0], Math.Max(p2[0], p3[0])), Math.Max(p1[1], Math.Max(p2[1], p3[1])),
